...
class DataIngestorASE2:
    ...
    def get_files_to_process(self, folder_path: str) -> Dict[str, List[str]]:
        ...
        try:
            logging.info(f"Analizando archivos en carpeta: {folder_path}")
            ...
            logging.info(f"Archivos CSV encontrados: {len(all_files)}")
            if len(all_files) > 0:
                logging.info(f"Primeros archivos: {all_files[:5]}")
            ...
            try:
                with self.engine.connect() as conn:
                    ...
                    if not table_exists:
                        logging.warning(f"Tabla de control {self.control_table} no existe. Todos los archivos serán considerados nuevos.")
                    ...
                    logging.info(f"Archivos procesados en BD: {len(processed)}")
                    logging.info(f"Archivos fallidos en BD: {len(failed)}")
                    
            except Exception as db_error:
                logging.error(f"Error consultando base de datos: {str(db_error)}")
            ...
            logging.info(f"Clasificación final - Nuevos: {len(new_files)}, Fallidos: {len(failed_files)}, Procesados: {len(processed_files)}")
