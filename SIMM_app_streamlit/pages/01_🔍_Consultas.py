...
# CONFIGURACIÓN GENERAL
# ==============================================
st.set_page_config(page_title="SIAMM - Consultas", page_icon="src/utils/favicon-114x114.png", layout="wide")
...
def sidebar_navegacion():
    with st.sidebar:
        ...
        st.caption("Seleccione la tabla a consultar")
        opcion = st.radio(
            "Tablas disponibles:",
...
def main():
    ...
    with st.container():
        if opcion_seleccionada == "Gestiones":
            st.title("Consultas de Gestiones")
            st.write("Seleccione los filtros y parámetros para consultar las gestiones...")
        elif opcion_seleccionada == "SMS":
            st.title("Consultas de SMS")
            st.write("Contenido de SMS...")
        elif opcion_seleccionada == "Pagos":
