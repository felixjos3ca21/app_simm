        ...
        with st.container():
            if self.modulo:
                ...
                uploaded_files = st.file_uploader(
                    ...
                    help=f"Seleccione archivo{'s' if multiple else ''} {'Excel' if tipo == 'xlsx' else 'texto'}",
                    accept_multiple_files=multiple
                )
                
                # Para módulos que no son pagos, mantener compatibilidad con el código existente
                if not multiple and uploaded_files:
                    return uploaded_files  # Devuelve el único archivo subido
                elif multiple and uploaded_files:
                    return uploaded_files  # Devuelve la lista de archivos
                ...
                if not processor.df_errores.empty:
                    with st.expander("🚨 Detalle de Errores", expanded=False):
                        ...
                        st.download_button(
                            label="📥 Descargar errores",
                            data=csv,
                            file_name=f"errores_{current_file.name}.csv",
                            mime="text/csv"
                        )
# ==============================================================================
# EJECUCIÓN PRINCIPAL
